...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/22 19:58:19 by mhervoch          #+#    #+#             */
/*   Updated: 2023/11/22 19:58:56 by mhervoch         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_line(char	*line, int fd, char *str, int *i)
{
	...
	if (!buffer)
		return (freeall(str, line, buffer));
	*i = read(fd, buffer, BUFFER_SIZE);
	if (*i < 0)
		return (freeall(line, str, buffer));
	if (*i == 0)
		free(buffer);
	else
	{
